...
Func Main ()
   ...
   ;----------------------------------------
   ;SELECT VEHICLE
   For $iYearCount = 2018 To 1996 Step -1
	  $aSelectData = YearVehicleSelectData($oIE, $iYearCount)
	  Exit
	  For $iYMMECount = 0 To UBound($aSelectData, $UBOUND_ROWS) - 12])
...
;                  RETURN			   :
;====================================================================================================================
Func YearVehicleSelectData(Byref $oIE, $sYear)
   ConsoleWrite ('YearVehicleSelectData' & @CRLF)
   Local $aSelectData [0][4]
