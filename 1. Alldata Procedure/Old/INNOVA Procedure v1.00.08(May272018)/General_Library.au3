...
;====================================================================================================================
Func Check_Subscription_Alldata ($oIE)
...
Func Write_Log_File ($sFilePath, $sWhichLogFile,  $sTxt, $sMode)
   ...
   Else ;$sWhichLogFile = "Log File Procedure Failed"
	  If $sMode = "overwrite" Then
...
;====================================================================================================================
Func Download_Procedure_Image_Collection ($sFilePath, $oIE)
ConsoleWrite ("Vao function tai hinh anh" & @CRLF)
   ...
   While $aImage_Links [$i] <> ""
	  ...
	  $i = $i + 1
   WEnd
ConsoleWrite ("bat dau function nho trong tai hinh anh" & @CRLF)
   ;Tải tất cả hình ảnh cần cho procedure với độ phân giải cao
   If ($bImage_Download = 1) and ($aProcedure_Image_Links [0] <> "") Then Image_Download_Convert2JPG($sFilePath, $aProcedure_Image_Links)
EndFunc
...
;                  OUTPUT              : AN JPG IMAGE
;====================================================================================================================
Func Image_Download_Convert2JPG($sFilePath, $aProcedure_Image_Links)
   ;Function này dùng để tải về hình ảnh và lưu vào vị trí FilePath
ConsoleWrite ("bat dau dowwnlad" & @CRLF)
   ;---------------------------------------------------
   ;START DOWNLOADING ALL IMAGES
   Local $ahDownload [1000]
   Local $i = 0
   While $aProcedure_Image_Links [$i] <> ""
	  ;Cắt phần tên hình ảnh của link để làm tên lưu cho hình ảnh
	  Local $iTrim_Pos = StringInStr ($aProcedure_Image_Links [$i], "/", 0, -1)
	  Local $ImageName
	  $ImageName = StringRight ($aProcedure_Image_Links [$i], StringLen($aProcedure_Image_Links [$i]) - $iTrim_Pos)
	  $ImageName = StringLeft($ImageName, StringLen ($ImageName) - 4) & ".jpg"
	  If ($bImage_Download = 1) Then
		 ;Download the file in the background with the selected option of 'force a reload from the remote site.'
		 $ahDownload [$i] = InetGet($aProcedure_Image_Links [$i], $sFilePath &"\"& $ImageName, $INET_FORCERELOAD, $INET_DOWNLOADBACKGROUND)
	  EndIf
	  $i = $i + 1
   WEnd
   ;---------------------------------------------------
   ;CHECK DOWNLOAD STATUS
ConsoleWrite ("check status" & @CRLF)
   Local $bNoti_Replace_Previous_Flag = False
   Local $i = 0
   Local $iTotalSize = 0
   Do
	  Sleep (500)
	  Local $bDownload_Done_Flag = True
	  $iTotalSize = 0
	  While $aProcedure_Image_Links [$i] <> ""
		 Local $hDownload = $ahDownload [$i]
		 If InetGetInfo($hDownload, $INET_DOWNLOADERROR) Then MsgBox (0, "", "Error")
		 Local $sDownloadRead = InetGetInfo($hDownload, $INET_DOWNLOADREAD)
		 $iTotalSize = $iTotalSize + $sDownloadRead
		 If InetGetInfo($hDownload, $INET_DOWNLOADCOMPLETE) = False Then $bDownload_Done_Flag = False
		 $i = $i + 1
	  WEnd
	  If $bNoti_Replace_Previous_Flag = False Then
		 Notification ("Downloading all images in the page -- " & @CRLF & $iTotalSize & "/Unknown (bytes)", "Normal")
		 $bNoti_Replace_Previous_Flag = True
	  Else
		 Notification ("Downloading all images in the page --" & @CRLF & $iTotalSize & "/Unknown (bytes)", "Replace Previous")
	  EndIf
   Until $bDownload_Done_Flag = True
   Sleep (10000)
   ;---------------------------------------------------
ConsoleWrite ("dong handle" & @CRLF)
   ;CLOSE ALL DOWNLOAD HANDLES
   Local $i = 0
   While $aProcedure_Image_Links [$i] <> ""
	  Sleep (50)
	  Local $hDownload = $ahDownload [$i]
	  ...
	  $i = $i + 1
   WEnd
   Notification ("Downloaded all images in the page -- " & $iTotalSize & " (bytes)", "Replace Previous")
...
;====================================================================================================================
Func Create_Procedure_Name ($oIE_Procedure)
   ;ĐOẠN CODE LẤY CÁC ID TRONG URL ĐỂ ĐẶT TÊN CHO PROCEDURE
   Local $sIE_Procedure_URL = _IEPropertyGet ($oIE_Procedure, "locationurl")
...
Func IENavigate_Check_Error (ByRef $oIE, $sLink_DTC)
   Do
ConsoleWrite ("Begin navigate" & @CRLF)
	  __IELockSetForegroundWindow($LSFW_LOCK)
	  _IENavigate ($oIE, $sLink_DTC, 1)
