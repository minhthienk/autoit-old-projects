...
Func Open_DTC_Link (Byref $oIE, $sSearch_String)
   ...
   If StringInStr ($sHTML_Innertext, "Page not found") <> 0 Or StringInStr ($sHTML_Innertext, "DOCTYPE html PUBLIC") <> 0 Or StringInStr ($sHTML_Innertext, "The page you requested can not be displayed") <> 0  Then IENavigate_Check_Error ($oIE, $sLink_YMME)
   ;-------------------------------------
   Do
	  ...
	  If _IEPropertyGet ($oIE, "title") = "ALLDATA Repair - Search Results" Then
		 ...
		 If $oLink = "" Then
			Notification ("Vehicle selected in IE does not match the config file" & @CRLF & "Please select the correct vehicle and RESTART the application!","Normal")
			While 1
			WEnd
		 EndIf
		 ;------------------------------------
