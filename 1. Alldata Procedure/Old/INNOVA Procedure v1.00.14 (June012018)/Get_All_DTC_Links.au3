...
Func Scan_DTCs ()
   ...
   If StringInStr ($sTxt_Title, "ALLDATA Repair - Vehicle Information - ") <> 0 Then
	  ...
	  Else
		 ...
		 ;GET SEARCH STRINGS
		 Local $sConfig_SearchStrings = StringRight ($sConfig, StringLen($sConfig) - StringInStr ($sConfig, "<<<-- SEARCH STRINGS -->>>") - StringLen ("<<<-- SEARCH STRINGS -->>>" & @CRLF))
		 ...
		 For $vElement In $aConfig_SearchStrings
			...
			If $D1 = $D1_Max Then $D1_Max = $D1 + 1
			If $D2 = $D2_Max Then $D2_Max = $D2 + 1
			ReDim $aSearch_Strings [$D1_Max][$D2_Max]
			   ...
			   If Check_Log_File ($sYMME, "Log File DTC Successful.txt", $sLink_DTC) = "Not Exist" Then
				  ...
				  If $j = 0 Then
					 ...
					 If $i = 0 Then
						...
						EndIf
					 Elseif $i = $D1_Max - 1 Then
						If StringLeft($aSearch_Strings [$i][0],5) = StringLeft($aSearch_Strings [$i - 1][0],4) Then
						   $sSub_Name = " (" & $sSystem_String & ")"
						EndIf
					 Else
						If StringLeft($aSearch_Strings [$i][0],5) = StringLeft($aSearch_Strings [$i + 1][0],5) Or StringLeft($aSearch_Strings [$i][0],5) = StringLeft($aSearch_Strings [$i - 1][0],4) Then
						   $sSub_Name = " (" & $sSystem_String & ")"
						EndIf
					 EndIf
