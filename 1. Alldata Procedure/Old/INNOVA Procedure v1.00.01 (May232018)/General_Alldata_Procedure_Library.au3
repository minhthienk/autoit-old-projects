...
Func Check_Login_Alldata ($oIE)
   ...
   If StringInStr ($sTxt_Body, "HTTP Status 404 - No result", 0, 1) <> 0 Or StringInStr ($sTxt_Body, "Please Log In", 0, 1) <> 0 Then
	  ;Đóng trang DTC hiện tại (chưa đăng nhập)
	  _IEQuit($oIE)
	  ;Mở trang login
	  Local $oIE_Login = _IECreate($sLink_Login,0,0);
	  ;Lấy object form login
	  Local $oForm = _IEFormGetObjByName($oIE_Login, "customer_login_center")
...
;====================================================================================================================
Func Check_Subscription_Alldata ($oIE, $sLink, $iSubscription_Num)
   Local $sResult
   Do
...
Func Insert_Images_HTML ($oIE, $sTxt_Body)
   ...
   While StringInStr ($sTxt_Body, "Zoom and Print Options", 0, 1) <> 0
	  ...
	  $sTemp = $sTemp & """"
	  $sTemp = "<br>" & @CRLF & "<img src=" & $sTemp & "/><br>"
...
Func Write_Log_File ($sFilePath, $sWhichLogFile,  $sTxt, $sMode)
   If $sWhichLogFile = "Log File DTC" Then
	  If $sMode = "overwrite" Then
		 Local $hFileOpen = FileOpen ($sFilePath & "\" & "Log File DTC" & ".txt",$FO_OVERWRITE)
	  Else
		 Local $hFileOpen = FileOpen ($sFilePath & "\" & "Log File DTC" & ".txt",$FO_APPEND)
	  EndIf
	  FileWrite($hFileOpen, $sTxt & @CRLF & @CRLF)
   Else
	  If $sMode = "overwrite" Then
		 Local $hFileOpen = FileOpen ($sFilePath & "\" & "Log File Procedure" & ".txt",$FO_OVERWRITE)
	  Else
		 Local $hFileOpen = FileOpen ($sFilePath & "\" & "Log File Procedure" & ".txt",$FO_APPEND)
	  EndIf
	  FileWrite($hFileOpen, $sTxt & @CRLF & @CRLF)
   EndIf
EndFunc
