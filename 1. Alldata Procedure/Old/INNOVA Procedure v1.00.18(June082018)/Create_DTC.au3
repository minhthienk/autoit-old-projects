...
Func DTC_Procedure_Alldata ($oIE, $sInsert_Path, $sSub_Name, $sMode)
   ...
   $sTxt_Body = Modify_Body_HTML ($oIE, "DTC")
   ;------------------------------------------------------------------------------------------------------------------
   ...
   If $sMode = "Not Main" Then
	  ...
	  Local $sFilePath_DTC         = @ScriptDir & "\INNOVA Prepair Procedures"      &"\"&$sYMME      &"\PROCEDURE"
	  If FileExists ($sFilePath_DTC) = 0 Then DirCreate($sFilePath_DTC)
	  Local $sFilePath_Title       = @ScriptDir & "\INNOVA Prepair Procedures"      &"\"&$sYMME      &"\PROCEDURE"       &"\"& $sTxt_Folder
   ...
   Else ;Main
	  ...
	  If FileExists ($sFilePath_DTC) = 0 Then DirCreate($sFilePath_DTC)
	  Local $sFilePath_Title       = @ScriptDir & "\INNOVA Prepair Procedures"      &"\"&$sYMME      &"\DTC"       &"\"& $sTxt_Folder
	  If FileExists ($sFilePath_Title) = 0 Then	DirCreate($sFilePath_Title)
   EndIf
   ...
   While $aHyperlink_Links [$iCount_Hyperlink] <> ""
	  ;------------------------------------------------------------------------------------------------------------------
	  ;CHECK IF THE LINK ALREADY EXISTED OR NOT
	  If Check_Log_File ($sYMME, "Log File Procedure Successful.txt", $aHyperlink_Links [$iCount_Hyperlink]) = "Not Exist" Or StringInStr ($aHyperlink_Links [$iCount_Hyperlink], "repair.alldata.com", 0, 1) = 0  Then
		 ...
		 Else
			...
			Local $sProcedure_Path = Create_JAVASCRIPT_Procedure ($oIE, $sFilePath_YMME, $aHyperlink_Links [$iCount_Hyperlink], $sInfo, $sYMME)
			Local $sHTML_Procedure_Hyperlink = "<a href=""" & $sProcedure_Path & """ target=""_blank"">" & "(More info)" & "</a>"
			$sTxt_Body = StringReplace ($sTxt_Body, Standardize_String ($aHyperlink_Innertexts [$iCount_Hyperlink]),$sHTML_Procedure_Hyperlink, 1, 0)
		 EndIf
	  Else
	  ...
	  $iCount_Hyperlink = $iCount_Hyperlink + 1
   WEnd
   ;NẾU CÓ PATH THÌ CHÈN VÔ
   If $sInsert_Path <> "" Then
	  Local $sHTML_Procedure_Hyperlink = "<a href=""" & $sInsert_Path & """ target=""_self"">" & "CONTINUE" & "</a>"
