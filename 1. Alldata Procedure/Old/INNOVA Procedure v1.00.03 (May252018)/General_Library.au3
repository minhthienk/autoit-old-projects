...
Func Check_Login_Alldata ($oIE)
   ...
   If StringInStr ($sTxt_Body, "HTTP Status 404 - No result", 0, 1) <> 0 Or StringInStr ($sTxt_Body, "Please Log In", 0, 1) <> 0 Then
	  ...
	  _IEQuit($oIE)
	  ;Mở trang login
	  Local $oIE_Login = IECreate_Check_Error($sLink_Login,0,0);
...
Func Check_Subscription_Alldata ($oIE, $sLink, $iSubscription_Num)
   ...
   Do
	  ...
	  If $sResult <> "Already had subscription  - No need reload" Then
		 ;Reload trang DTC
		 _IEQuit ($oIE)
		 $oIE = IECreate_Check_Error ($sLink,  $bWeb_Attach, $bWeb_Visible, $bWeb_Wait, $bWeb_TakeFocus)
	  EndIf
...
Func Image_Download_Convert2JPG($sFilePath, $sLink)
   ...
   If ($bImage_Download = 1) Then
	  ...
	  Do
		 ...
		 Notification ("Downloading:  " & @CRLF & $ImageName & @CRLF & $sDownloadRead & "/" & $sDownloadSize & " (bytes)")
		 Sleep(500)
