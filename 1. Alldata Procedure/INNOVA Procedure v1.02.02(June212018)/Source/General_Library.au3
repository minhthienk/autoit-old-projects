...
;~ ConsoleWrite ("get ok button" & @CRLF)
			Local $oOK_Button = _IEGetObjById ($oIE, "ok_button")
			;Nhấn nút OK
;~ ConsoleWrite ("Click ok button" & @CRLF)
			_IEAction ($oOK_Button, "click" )
;~ ConsoleWrite ("get images" & @CRLF)
			;Chờ cho action done
			Sleep (1000)
...
Func Insert_Images_HTML ($oIE, $sTxt_Body)
   ...
   For $oTag In $oTags
	  If $oTag.GetAttribute("class") = "caption" And StringInStr ($sTxt_Body, $oTag.innertext) <> 0 Then
		 Local $sOrigin = "Zoom and Print Options" & "<br>" & @CRLF & "<br>" & @CRLF & $oTag.innertext & "<br>" & @CRLF
		 Local $sSwap = "<br>" & @CRLF & $oTag.innertext & "Zoom and Print Options" & "<br>" & @CRLF
		 $sTxt_Body = StringReplace($sTxt_Body, $sOrigin, $sSwap)
	  EndIf
   Next
...
;====================================================================================================================
Func Modify_Body_HTML ($oIE, $sMode, $sTitle = "Default")
   Sleep (300)
   ...
   ;------------------------------------------------------------------------------------------------------------------
   ;Định dạng Heading1 cho header
   If $sMode = "DTC" Then
	  $sTxt_Body = StringReplace ($sTxt_Body, "<br>" & @CRLF, "<br>" & @CRLF & "<h1>", 1)
	  $sTxt_Body = StringReplace ($sTxt_Body, "<br>" & @CRLF & "<br>" & @CRLF, "</h1><br>" & @CRLF & "<br>" & @CRLF, 1)
   EndIf
   ...
   For $i=0 To $iMax_Data_Bold Step 1
	  If $sMode = "DTC" Then
		 ;Check string dùng heading 2
		 ;Remove khoảng trắng
		 Local $sStringNoSpace = StringReplace ($aData_Bold[$i], " ", "")
		 ;Trường hợp CAUTION/HINT
		 $sStringNoSpace = StringReplace ($sStringNoSpace, "/", "")
		 ;Nếu string là viết hoa hoàn toàn và số lượng ký tự lớn hơn 4 (tránh OK, NG ...)
		 If StringIsUpper($sStringNoSpace) And StringLen ($sStringNoSpace) > 4 Then
			;Dấu hiệu thay thế: (Xuống dòng) + Bold Text & <br>
			$sTxt_Body = StringReplace ($sTxt_Body, "<br>" & @CRLF & "<br>" & @CRLF & $aData_Bold[$i] & "<br>" & @CRLF & "<br>" & @CRLF, "<br>" & @CRLF & "<br>" & @CRLF & "<center><h2>" & $aData_Bold[$i] & "</h2></center>" & @CRLF, 1)
			$sTxt_Body = StringReplace ($sTxt_Body, "<br>" & @CRLF & "<br>" & @CRLF & $aData_Bold[$i] & "<br>" & @CRLF, "<br>" & @CRLF & "<br>" & @CRLF & "<center><h2>" & $aData_Bold[$i] & "</h2></center>" & @CRLF, 1)
			$sTxt_Body = StringReplace ($sTxt_Body, "</h2></center>" & @CRLF & $aData_Bold[$i] & "<br>" & @CRLF, "</h2></center>" & @CRLF & "<center><h2>" & $aData_Bold[$i] & "</h2></center>" & @CRLF, 1)
		 Else
			;Dấu hiệu thay thế: (Xuống dòng) + Bold Text & <br>
			$sTxt_Body = StringReplace ($sTxt_Body, @CRLF & $aData_Bold[$i] & "<br>", @CRLF&"<b>" & $aData_Bold[$i] & "</b><br>" , 1)
		 EndIf
	  Else ;$sMode = "Procedure"
			;Dấu hiệu thay thế: (Xuống dòng) + Bold Text & <br>
			$sTxt_Body = StringReplace ($sTxt_Body, @CRLF & $aData_Bold[$i] & "<br>", @CRLF&"<b>" & $aData_Bold[$i] & "</b><br>" , 1)
	  EndIf
   Next
   ...
   ;------------------------------------------------------------------------------------------------------------------
   ;ĐOẠN CODE THAY THẾ HEADER DTC DEFINITION CỦA ALLDATA THÀNH INNOVA
   If $sMode = "DTC" Then
	  Local $iStart_Header = StringInStr ($sTxt_Body, "<h1>")
	  Local $iEnd_Header1 = StringInStr ($sTxt_Body, "<center><h2>") - 1
	  If $iEnd_Header1 = -1 Then $iEnd_Header1 = 9999
	  Local $iEnd_Header2 = StringInStr ($sTxt_Body, "<b></b><br>" & @CRLF & "Continued") - 1
	  If $iEnd_Header2 = -1 Then $iEnd_Header2 = 9999
	  Local $iEnd_Header3 = StringInStr ($sTxt_Body, "<b></b><br>" & @CRLF & "&nbsp;") - 1
	  If $iEnd_Header3 = -1 Then $iEnd_Header3 = 9999
	  Local $iEnd_Header4 = StringInStr ($sTxt_Body, "<b></b><br>" & @CRLF & "continued") - 1
	  If $iEnd_Header4 = -1 Then $iEnd_Header4 = 9999
	  Local $iEnd_Header5 = StringInStr ($sTxt_Body, "<img") - 1
	  If $iEnd_Header5 = -1 Then $iEnd_Header5 = 9999
	  Local $iEnd_Header6 = StringInStr ($sTxt_Body, "HINT: ") - 1
	  If $iEnd_Header6 = -1 Then $iEnd_Header6 = 9999
	  Local $iEnd_Header7 = StringInStr ($sTxt_Body, "<br>" & @CRLF & " Zoom and Print Options") - 1
	  If $iEnd_Header7 = -1 Then $iEnd_Header7 = 9999
	  Local $iEnd_Header8 = StringInStr ($sTxt_Body, "<br>" & @CRLF & "Zoom and Print Options") - 1
	  If $iEnd_Header8 = -1 Then $iEnd_Header8 = 9999


	  Local $iEnd_Header = $iEnd_Header1
	  Local $aEnd_Headers [] = [$iEnd_Header1, $iEnd_Header2, $iEnd_Header3, $iEnd_Header4, $iEnd_Header5, $iEnd_Header6, $iEnd_Header7, $iEnd_Header8]
	  For $vElement In $aEnd_Headers
		 If $vElement < $iEnd_Header Then
			$iEnd_Header = $vElement
		 EndIf
	  Next

	  Local $sHeader = StringMid ($sTxt_Body, $iStart_Header, $iEnd_Header - $iStart_Header + 1)

	  Local $sDTC_Definition = DTC_Lookup ($sDTC_Database, $sDTC_MsgID & "_" & $sTitle)
	  If $sDTC_Definition = "No DTC Definition matched. Please ADD manually!!!" Then MsgBox ($MB_TOPMOST,"Message", "No DTC DEFINITION matched the DTC " & $sTitle

	  $sTxt_Body = StringReplace ($sTxt_Body, $sHeader, "<h1>" & $sTitle & ": " & $sDTC_Definition & "</h1>" & @CRLF)
   EndIf


   ;------------------------------------------------------------------------------------------------------------------
   ;ĐOẠN CODE THAY THẾ CÁC STRING OEM TRONG ALLDATA THÀNH STRING INNOVA
   $sTxt_Body = StringReplace ( $sTxt_Body, "Techstream", "Scan Tool", 0, 1)
   $sTxt_Body = StringReplace ( $sTxt_Body, "TECHSTREAM", "SCAN TOOL", 0, 1)
   $sTxt_Body = StringReplace ( $sTxt_Body, "TechStream", "Scan Tool", 0, 1)
   ;Thêm khoảng trắng sau các mục (a) ...
   For $i = 33 To 126
	  $sTxt_Body = StringReplace ( $sTxt_Body, "(" & Chr($i) & ")", "(" & Chr($i) & ") ", 0, 1)
   Next

   ;------------------------------------------------------------------------------------------------------------------
   ;IN ĐẬM CÁC MỤC
   Local $sTxt = $sTxt_Body
   $sTxt = StringReplace ($sTxt, "<br>", "")
   $sTxt = StringReplace ($sTxt, "<b>", "")
   $sTxt = StringReplace ($sTxt, "</b>", "")
   ;Replace xuống dòng nhiều lần bằng tối đa 1 lần xuống dòng trong
   Do
	  $sTxt = StringReplace ( $sTxt, @CRLF&@CRLF, @CRLF)
   Until @extended = 0

   Local $aSingle_Line = StringSplit ($sTxt, @CRLF, $STR_ENTIRESPLIT)
   ;Check string by string in the array if it maps the condition
   For $i = 1 to $aSingle_Line [0]
	  ;Nếu string có dạng chỉ mục (có dấu ".")
	  Local $Element = $aSingle_Line [$i]
			;Cắt các khoảng trắng thừa bên trái
			While StringLeft ($Element, 6) = "&nbsp;"
			   $Element = StringTrimLeft ($Element, 6)
			WEnd
	  If StringInStr ($Element, ". ", 0, 1) > 0 And StringInStr ($Element, ". ", 0 ,1) < 5 Then
		 Local $sTemp = $aSingle_Line [$i]
			;Cắt các khoảng trắng thừa bên trái
			While StringLeft ($sTemp, 1) = " "
			   $sTemp = StringTrimLeft ($sTemp, 1)
			WEnd
			...
			While StringRight ($sTemp, 1) = " "
			   $sTemp = StringTrimRight ($sTemp, 1)
			WEnd
		 $sTxt_Body = StringReplace ($sTxt_Body, $sTemp, "<b>" & $sTemp & "</b>", 1)
	  EndIf
   Next
   ;------------------------------------------------------------------------------------------------------------------
   ;IN ĐẬM HINT:
   $sTxt_Body = StringReplace ( $sTxt_Body, "HINT:", "<b> HINT:</b>")
...
Func Download_Procedure_Image_Collection ($FilePath, $oIE)
   ...
   $i = 0
   ;Kiểm tra mạng không ổn định thì tải lại
   While $aProcedure_Image_Links [$i] <> ""
	  If Image_Download_Convert2JPG($FilePath, $aProcedure_Image_Links [$i]) = True Then
		 $i = $i + 1
	  Else
		 $i = $i - 1
	  EndIf
...
Func Image_Download_Convert2JPG($sFilePath, $sLink)
   ...
   If ($bImage_Download = 1) Then
	  ...
	  Do
		 ...
		 Sleep(200)
		 $iTime_out += 200
		 If $iTime_out >= 30000 + $sDownloadRead/4  Then
			$bResult = False
			ExitLoop
		 EndIf
	  ...
	  Else
		 Notification ("WARNING: Low connection" & "This can cause image crashes" & @CRLF & "Restarting the image download ..." , "Normal")
		 Sleep (500)
	  EndIf
   EndIf
   Return $bResult
