...
Global $sVersion = "Procedure Generator v1.02.01"
...
#Region ### START GUI section ### Form=
   ...
   GUICtrlCreateGroup("Settings", 10, 140, 309, 110)
	  ...
	  ;Select Database
	  $Combo_DTC_Database = GUICtrlCreateCombo("(Select DTC Database)", 152, 185, 160, 25, BitOR($CBS_DROPDOWN,$CBS_AUTOHSCROLL))
	  GUICtrlSetData(-1, "TOYOTA DTC DEF_ ENG.xlsx|TOYOTA DTC DEF.xlsx")
	  GUICtrlSetOnEvent($Combo_DTC_Database, "Select_MsgID")

	  ;Select MessageId
	  $Combo_DTC_MsgID = GUICtrlCreateCombo("(Select DTC MessageID)", 152, 215, 160, 25, BitOR($CBS_DROPDOWN,$CBS_AUTOHSCROLL))
	  GUICtrlSetState ($Combo_DTC_MsgID, $GUI_DISABLE)
...
Func Select_MsgID()
	  If GUICtrlRead ($Combo_DTC_Database) = "TOYOTA DTC DEF_ ENG.xlsx" Then
			GUICtrlSetState ($Combo_DTC_MsgID, $GUI_ENABLE)
			GUICtrlSetData($Combo_DTC_MsgID, "")
			GUICtrlSetData($Combo_DTC_MsgID, "(Select DTC MessageID)" & "|" & "61|234|372")
			_GUICtrlComboBox_SetCurSel($Combo_DTC_MsgID, 0)

		 ElseIf GUICtrlRead ($Combo_DTC_Database) = "TOYOTA DTC DEF.xlsx" Then
			GUICtrlSetState ($Combo_DTC_MsgID, $GUI_ENABLE)
			GUICtrlSetData($Combo_DTC_MsgID, "")
			GUICtrlSetData($Combo_DTC_MsgID, "(Select DTC MessageID)"  & "|" & "61|234")
			_GUICtrlComboBox_SetCurSel($Combo_DTC_MsgID, 0)
		 Else
			GUICtrlSetState ($Combo_DTC_MsgID, $GUI_DISABLE)
	  EndIf
...
Func Wait_Key ()
   ...
   Notification ("Please enter a VALID KEY to continue using the app!", "Normal")
   While 1
	  If $bKeySubmit_Flag = True Then KeySubmit ()
	  If $bTotal_Allow_Flag = True Then
		 GUI_Input_Ctrl_Set_State ("Enable")
		 GUICtrlSetData ( $Status, "Standby")
		 ExitLoop
	  EndIf
   WEnd
EndFunc
...
Func Check_Init_Vals ($sWhich_Tab)
   ...
   If $sWhich_Tab <> "Write Config" Then
	  ;-------------------------------------------------
	  ;CHECK DTC DATABASE
	  Local $sCombo_DTC_Database_Val = GUICtrlRead ($Combo_DTC_Database)
	  Local $bDTC_Database_Flag = False
	  If $sCombo_DTC_Database_Val = "(Select DTC Database)" Then
			Notification ("Please select DTC DATABASE!", "Normal")
			$bDTC_Database_Flag = False
		 Else
			$sDTC_Database = $sCombo_DTC_Database_Val
			$bDTC_Database_Flag = True
	  EndIf
	  ;-------------------------------------------------
	  ;CHECK DTC MESSAGE ID
	  Local $sCombo_DTC_MsgID_Val = GUICtrlRead ($Combo_DTC_MsgID)
	  Local $bDTC_MsgID_Flag = False
	  If $sCombo_DTC_MsgID_Val = "(Select DTC MessageID)" Then
			Notification ("Please select DTC MESSAGE ID!", "Normal")
			$bDTC_Database_Flag = False
		 Else
			$sDTC_MsgID = $sCombo_DTC_MsgID_Val
			$bDTC_Database_Flag = True
	  EndIf
   EndIf
   ...
   ;-------------------------------------------------
   ;SET FLAG
   If ($bSub_Flag = True) And ($bLink_Flag = True) And ($bDTC_Database_Flag = True) And ($bDTC_Database_Flag = True) Then
...
Func GUI_Input_Ctrl_Set_State ($sState)
   If $sState = "Disable" Then
	  ...
	  GUICtrlSetState ($Combo_DTC_Database, $GUI_DISABLE)
	  GUICtrlSetState ($Combo_DTC_MsgID, $GUI_DISABLE)
   ...
   Else
	  ...
	  GUICtrlSetState ($Combo_DTC_Database, $GUI_ENABLE)
	  GUICtrlSetState ($Combo_DTC_MsgID, $GUI_ENABLE)
